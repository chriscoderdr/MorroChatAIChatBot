...
import { AgentResult, AgentContext, AgentName } from '../../types';
...
const mockChatSessionModel: Partial<Model<ChatSession>> = {
  ...
  create: jest.fn().mockResolvedValue({ sessionId: 'test' } as any),
...
describe('Agent Tests', () => {
  ...
  describe('Weather Agent', () => {
    it('should return weather information', async () => {
      const response = await testAgent(
        ...
        /(temperature|forecast|weather)/i,
      );
      expect(response).toBeDefined();
      expect(response.output).toBeDefined();
    });
  ...
  describe('Research Agent', () => {
    ...
    it('should handle follow-up questions', async () => {
      ...
      const followupResponse = await testAgent(
        'research',
        'How old is Joe Biden the US president?',
        'research-followup-session',
        undefined,
        true,
      );
    ...
    it('should not leak thought processes in responses', async () => {
      const response = await testAgent(
        ...
        'research-leakage-session',
      );
      expect(response.output).not.toMatch(
        /\bTHOUGHT\b|\bACTION\b|\bOBSERVATION\b/i,
      );
  ...
  describe('Topic Filtering', () => {
    it('should filter inappropriate topics', async () => {
      ...
      expect(response.output).toMatch(
        /do not have|cannot provide|not available|unable to|don't have/i,
      );
    });
  });
  ...
  describe('Agent Router', () => {
    it('should route to the correct agent based on input', async () => {
      ...
      const weatherResponse = await AgentRegistry.callAgent(
        'open_weather_map',
        weatherInput,
        { sessionId: 'router-test', chatHistory: [], input: weatherInput },
      );
      const timeResponse = await AgentRegistry.callAgent('time', timeInput, {
        sessionId: 'router-test',
        chatHistory: [],
        input: timeInput,
      });
      const researchResponse = await AgentRegistry.callAgent(
        'research',
        researchInput,
        { sessionId: 'router-test', chatHistory: [], input: researchInput },
      );
      const documentResponse = await AgentRegistry.callAgent(
        'document_search',
        documentInput,
        { sessionId: 'router-test', chatHistory: [], input: documentInput },
      );

      expect(weatherResponse.output).toMatch(
        /(temperature|forecast|weather|Could not find location|API returned error)/i,
      );
      expect(timeResponse.output).toMatch(
        /(current time|is \d{1,2}:\d{2}|need to know|location)/i,
      );
