...
import { AgentContext, AgentName } from '../types';
    ...
    if (!existingSessionCookie) {
      ...
      if (cookies && Array.isArray(cookies) && cookies.length > 0) {
        const sessionCookie = cookies.find((cookie) =>
          cookie.includes('browserSessionId'),
        );
        if (sessionCookie) {
...
#HttpOnly_localhost\tFALSE\t/\tFALSE\t${Math.floor(Date.now() / 1000) + 86400 * 30}\tbrowserSessionId\t${browserSessionId}`;
  ...
  describe('Chat API Tests', () => {
    ...
    const sendChatMessage = async (
      ...
      if (shouldLog) {
        testLogger.log(
          `Response: "${response.body.reply.substring(0, 100)}${response.body.reply.length > 100 ? '...' : ''}"`,
        );
      }
    ...
    describe('Time Agent Tests', () => {
      it('should correctly respond to time questions', async () => {
        const reply = await sendChatMessage(
          'What time is it in Santo Domingo, Dominican Republic?',
        );

        // Should contain time information and location reference
        expect(reply.toLowerCase()).toMatch(
          /\b(time|santo domingo|dominican|tuesday|july)\b/,
        );
      ...
      it('should handle time in other locations', async () => {
        const reply = await sendChatMessage(
          'What time is in Bangkok, Thailand?',
        );
      ...
      it('should respond to time questions via curl with cookies', () => {
        try {
          ...
          const jsonResult: CurlResponse = JSON.parse(result);
          ...
          testLogger.log(
            `Curl test passed: ${jsonResult.reply.substring(0, 50)}...`,
          );
        } catch (error: any) {
    ...
    describe('Weather Agent Tests', () => {
      it('should correctly respond to weather questions', async () => {
        const reply = await sendChatMessage(
          "How's the weather in Santo Domingo, Dominican Republic?",
        );
      ...
      it('should respond to weather questions via curl with cookies', () => {
        try {
          ...
          testLogger.log(
            `Curl weather test passed: ${jsonResult.reply.substring(0, 50)}...`,
          );
        } catch (error: any) {
    ...
    describe('Research Agent Tests', () => {
      ...
      it('should handle follow-up questions', async () => {
        ...
        expect(reply.toLowerCase()).toMatch(
          /\b(founder|founded|established|amazon|jeff|bezos)\b/i,
        );
        expectNoAgentThoughtProcess(reply);
      }, 30000);
      ...
      it('should handle questions about company location', async () => {
        ...
        expect(reply.toLowerCase()).toMatch(
          /\b(located|boston|massachusetts|location|address)\b/i,
        );
        expectNoAgentThoughtProcess(reply);
      }, 30000);
      ...
      it('should research via curl with cookies', () => {
        try {
          ...
          testLogger.log(
            `Curl research test passed: ${jsonResult.reply.substring(0, 50)}...`,
          );
        } catch (error: any) {
    ...
    describe('Topic Filtering Tests', () => {
      ...
      it('should reject questions not related to the default topic if topic filtering is enabled', async () => {
        ...
        if (topic !== 'No topic') {
          const unrelatedTopic =
            topic === 'Dominican Food' ? 'beisbol' : 'Quantum Physics';
          const reply = await sendChatMessage(
            `Talk me about ${unrelatedTopic}`,
          );

          // If topic filtering is enabled, it should refuse
          if (
            reply.toLowerCase().includes('not related to') ||
      ...
      it('should enforce topic filtering via curl with cookies', () => {
        const topic = getTopic();
        if (topic !== 'No topic') {
          try {
            ...
            testLogger.log(
              `Curl topic test passed: ${jsonResult.reply.substring(0, 50)}...`,
            );
          } catch (error: any) {
  ...
  describe('Document Agent Tests', () => {
    ...
    it('should upload and process a PDF document', async () => {
      ...
      if (!fs.existsSync(pdfPath)) {
        testLogger.warn(
          'sample_account_details.pdf not found, skipping document tests',
        );
        return;
      }
    ...
    it('should answer questions about the uploaded document', async () => {
      if (!pdfUploaded) {
        testLogger.warn(
          'PDF was not uploaded successfully, skipping document question test',
        );
        return;
      }
    ...
    it('should describe document details', async () => {
      if (!pdfUploaded) {
        testLogger.warn(
          'PDF was not uploaded successfully, skipping document details test',
        );
        return;
      }
    ...
    it('should extract specific information about ACH routing', async () => {
      if (!pdfUploaded) {
        testLogger.warn(
          'PDF was not uploaded successfully, skipping document extraction test',
        );
        return;
      }
    ...
    it('should extract information about the bank in the file', async () => {
      if (!pdfUploaded) {
        testLogger.warn(
          'PDF was not uploaded successfully, skipping bank name extraction test',
        );
        return;
      }
    ...
    it('should answer document questions via curl with cookies', () => {
      if (!pdfUploaded) {
        testLogger.warn(
          'PDF was not uploaded successfully, skipping curl document test',
        );
        return;
      }
      ...
      try {
        ...
        expect(jsonResult.reply).toBeDefined();
        expect(jsonResult.reply).toMatch(
          /\b(account|bank|document|details)\b/i,
        );
        expectNoAgentThoughtProcess(jsonResult.reply);

        testLogger.log(
          `Curl document test passed: ${jsonResult.reply.substring(0, 50)}...`,
        );
      } catch (error: any) {
  ...
  describe('Agent Registry', () => {
    it('should have all required agents registered', () => {
      ...
      requiredAgents.forEach((agentName) => {
        const agent = AgentRegistry.getAgent(agentName as AgentName);
      ...
      allAgents.forEach((agent) => {
        testLogger.log(
          `- ${agent.name}: ${agent.description.substring(0, 50)}...`,
        );
      });
    });
  });
  ...
  describe('Research Agent', () => {
    it('should respond to research questions', async () => {
      ...
      if (agent) {
        ...
        testLogger.log(
          `Research agent output: "${result.output.substring(0, 50)}..."`,
        );
      }
    }, 30000);
    ...
    it('should handle follow-up questions with context', async () => {
      ...
      if (agent) {
        ...
        const mockContext: AgentContext = {
          ...
          chatHistory: [
            new HumanMessage('When was Soluciones GBH founded?'),
            new AIMessage(
              'Soluciones GBH was founded in 2004 in the Dominican Republic.',
            ),
          ],
        };
        ...
        expect(result.output.toLowerCase()).toMatch(
          /\b(founder|founded|established|creator)\b/i,
        );
        expectNoAgentThoughtProcess(result.output);
        testLogger.log(
          `Research follow-up output: "${result.output.substring(0, 50)}..."`,
        );
      }
    }, 30000);
  });
...
`;
        ...
        expect(result.output).toBeDefined();
        expect(result.output.toLowerCase()).toMatch(
          /\b(bill gates|paul allen|1975|microsoft)\b/i,
        );
        expectNoAgentThoughtProcess(result.output);
      }
    });
  ...
  describe('Web Search Agent', () => {
    it('should respond to web search queries', async () => {
      ...
      testLogger.log(
        `Web search output length: ${outputText.length} characters`,
      );
    }, 30000);
  });
});
