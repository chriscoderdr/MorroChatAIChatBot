...
export class MongoDBChatMessageHistory extends BaseListChatMessageHistory {
  ...
  async getMessages(): Promise<BaseMessage[]> {
    ...
    console.log(`MongoDBChatMessageHistory: getMessages for session ${this.sessionId} - found ${storedMessages.length} messages`);
  ...
  async addMessage(message: BaseMessage): Promise<void> {
    ...
    console.log(`MongoDBChatMessageHistory: addMessage for session ${this.sessionId} - type: ${message._getType()}`);
    ...
    try {
      ...
      console.error(`MongoDBChatMessageHistory: Error adding message for session ${this.sessionId}:`, error);
      throw error;
    }
  }
  ...
  async debugMessageCount(): Promise<number> {
    ...
    console.log(`MongoDBChatMessageHistory: Session ${this.sessionId} has ${count} messages`);
    return count;
  }
  ...
  async debugRecentMessages(limit: number = 3): Promise<void> {
    ...
    if (!session || !session.messages || session.messages.length === 0) {
      console.log(`MongoDBChatMessageHistory: No messages found for session ${this.sessionId}`);
      return;
    }
    
    const messages = session.messages.slice(-limit);
    console.log(`MongoDBChatMessageHistory: Last ${messages.length} messages for session ${this.sessionId}:`);
    
    messages.forEach((msg: any, i: number) => {
      const type = msg.type || 'unknown';
      const contentPreview = typeof msg.data?.content === 'string' 
        ? msg.data.content.substring(0, 50) + (msg.data.content.length > 50 ? '...' : '')
        : 'No content available';
      console.log(`[${i + 1}/${messages.length}] Type: ${type}, Content: ${contentPreview}`);
    });
  }
}