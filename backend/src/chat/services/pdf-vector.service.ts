...
export class PdfVectorService {
  ...
  async vectorizeAndStorePdf(
    ...
    try {
      ...
      try {
        ...
        this.logger.log(
          `Last 200 characters: "...${text.substring(Math.max(0, text.length - 200))}"`,
        );
      } catch (pdfError: any) {
        ...
        try {
          ...
          text = pdfData.text;
          this.logger.log(
            `PDF parsed with lenient options. Text length: ${text.length} characters`,
          );
        } catch (fallbackError) {
          this.logger.error(
            `Both PDF parsing attempts failed. Standard: ${pdfError.message}, Fallback: ${(fallbackError as Error).message}`,
          );
          ...
          if (!pdfHeader.startsWith('%PDF')) {
            throw new Error(
              'File is not a valid PDF. Please upload a valid PDF file.',
            );
          }
          ...
          try {
            ...
            text = pdfData.text;
            this.logger.log(
              `PDF parsed with limited pages. Text length: ${text.length} characters`,
            );
          } catch (finalError: any) {
            throw new Error(
              `Invalid PDF structure or corrupted file. Unable to extract text from PDF: ${finalError.message}`,
            );
          }
        }
      }
      ...
      this.logger.log(
        `Text preprocessed. Original length: ${text.length}, Preprocessed length: ${preprocessedText.length}`,
      );
      ...
      const filteredDocs = docs.filter((doc, index) => {
        ...
        if (!isValid) {
          this.logger.debug(
            `Filtered out chunk ${index}: length=${content ? content.length : 0}, preview="${content ? content.substring(0, 50) : 'empty'}"`,
          );
        }
      ...
      docs = filteredDocs;
      this.logger.log(
        `After filtering (min 50 chars): ${docs.length} chunks (filtered out ${originalCount - docs.length})`,
      );
      ...
      if (docs.length > 0) {
        ...
        this.logger.log(
          `Chunk size stats: avg=${Math.round(avgChunkSize)}, min=${minChunkSize}, max=${maxChunkSize}`,
        );

        // Log first few chunk previews
        docs.slice(0, 3).forEach((doc, i) => {
          this.logger.log(
            `Chunk ${i} preview (${doc.pageContent.length} chars): "${doc.pageContent.substring(0, 100)}..."`,
          );
        });
      }
      if (docs.length === 0) {
      ...
      const validEntries = docs
        ...
        .filter(
          (entry) =>
            ...
            entry.vector.some((v) => typeof v === 'number' && !isNaN(v)),
        );
      ...
      await collection.add({
        ids: validEntries.map(
          (_, i) =>
            `${collectionName}_doc_${Date.now()}_${validEntries[i].idx}`,
        ),
        ...
        metadatas: validEntries.map((entry, i) => ({
          ...
          wordCount:
            entry.doc.metadata?.wordCount ||
            entry.doc.pageContent.split(/\s+/).length,
          hasNumbers:
            entry.doc.metadata?.hasNumbers || /\d+/.test(entry.doc.pageContent),
          hasArticleReference:
            entry.doc.metadata?.hasArticleReference ||
            /article\s+\d+|artículo\s+\d+/i.test(entry.doc.pageContent),
      ...
      this.logger.log(
        `Successfully stored ${validEntries.length} chunks in ChromaDB collection: ${collectionName}`,
      );
      this.logger.log(
        `Total characters stored: ${validEntries.reduce((sum, entry) => sum + entry.doc.pageContent.length, 0)}`,
      );

      return {
        chunks: validEntries.length,
        filtered: docs.length - validEntries.length,
      };
    } catch (error) {
   ...
   */
  private postProcessChunks(docs: Document[]): Document[] {
  ...
  private startsWithHeader(content: string): boolean {
    ...
    if (
      /^(Article\s+\d+|Artículo\s+\d+|Section\s+\d+|Chapter\s+\d+)/i.test(
        firstLine,
      )
    ) {
      return true;
  ...
  private hasStrongSectionBreak(chunk1: string, chunk2: string): boolean {
    ...
    return (
      /CHAPTER|SECTION|ARTICLE|TÍTULO|CAPÍTULO|SECCIÓN|ARTÍCULO/i.test(
        start2,
      ) || /\n\n\n/.test(end1 + start2)
    );
  }
