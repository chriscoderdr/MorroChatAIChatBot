...
export class ChatService {
  ...
  async invoke(userMessage: string, userId: string): Promise<string> {
    try {
      ...
      const mongoHistory = new MongoDBChatMessageHistory(
      ...
      if (typeof result === 'string') {
        ...
        finalResponseContent = result.output;
      } else {
        this.logger.error(
          JSON.stringify({
            level: 'error',
      ...
      this.logger.error(
        `Error in chat service invoke: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async addDocumentContext(
    ...
    try {
      ...
      this.logger.log(
        `Added document context for user ${userId}: ${documentInfo}`,
      );
    } catch (error: any) {
      this.logger.error(
        `Error adding document context: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async processChat(
    ...
    console.log(
      `=== CHAT SERVICE: Processing message "${userMessage}" for user ${userId} ===`,
    );
